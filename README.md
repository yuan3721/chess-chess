# Chess Chess · 在先国际象棋（纯前端）

一个**单文件、零依赖、离线可用**的在线国际象棋页面。双击 `index.html` 即可开始对弈，无需后端、数据库、登录或联网。

内置 **Stockfish 19 Lite（WebAssembly）**——本地人机对战，全部离线运行。

## 快速开始

```bash
# 方式一：直接双击打开（AI 也开箱即用）
open index.html          # macOS 不能对战
# 方式二：任意静态服务器
...
- 引擎 wasm 以 base64 内嵌在 index.html 中，运行时还原为 Blob URL，通过 URL hash 协议注入 Worker —— 因此**双击打开、断网都能和 AI 下**
- UCI 通信：`UCI_LimitStrength`/`UCI_Elo` 控棋力，`go movetime` 控响应时间（0.25–1 秒）
- 代际号（epoch）机制：重开 / 悔棋时自动丢弃在途的 AI 思考结果，避免错乱
...
src/
  ...
  app.js            应用逻辑（模块化：Game / Engine / BoardView / …）
vendor/
...
| `Engine` | Stockfish Worker 封装（init / think / cancel） | 换引擎、调强度只改这里 |
...
- AI：file:// 离线启动、执白/执黑、5 档难度、思考中锁盘、悔棋语义、思考中途重开（代际丢弃）、升变解析、移动端视口
